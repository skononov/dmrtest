...
//decode bit sequence
int bercalc(const int *I, const int *Q, int size, int *numerr, int *numbit)
{
	*numbit = 0;
	*numerr = 0;
	...
	for (int i=0; i<size; i++) ph[i] = atancord(I[i], Q[i]);
