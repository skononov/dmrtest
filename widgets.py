...
class DTMainMenuFrame(tk.Frame):
    ...
    mb = self.chooseScenarioMB = tk.Menubutton(master=self, text='Выбрать сценарий')
    mb.configure(padx=10, pady=5)
    mb.grid(row=0)
    mb.menu = ChooseScenarioMenu(self.createScenarioMB)

    csb = self.createScenarioMB = tk.Button(master=self, text='Создать сценарий', 
            command=self.createScenario)
    mb.configure(padx=10, pady=5)
    csb.grid(row=1)
